...
void JK_statistics::read_data(int iJK, int TASK)
//start at iJK=0, NJK-1
{
  ...
  if(TASK!=1000) { //that means that we are really using the tasks
    ...
    file_in_GG = path_gen + "CORR_GG/ONE_TASK/CFw_snap_" + osSNAP.str() + "_sub_" + osSC.str() + "_JKsample_"
      + osJK.str() + "_task_" + osTASK.str() + "_binsM_" + osNM.str() + "_binsR_" + osNR.str() 
      + "_NT_" + osNTASK.str() + ".dat";
    
    file_in_SHgm = path_gen + "CORR_GG/DELTA_SIGMA/ONE_TASK/delta_sigma_gm_no_rhob_sub_" 
      + osSC.str() + "_JKsample_" + osJK.str() + "_one_task_" + osTASK.str() + ".dat";
  }
