...

{
  ...
  */    
  ...
  /*const string path_in_GM = path_gen + "CORR_GM/ONE_TASK/WEIGHTED/";
  const string file_in_GM = path_in_GM + "CFw_snap_" + osSNAP.str() + "_sub_" + argv[1]
    + "_binsM_" + osNM.str() + "_binsR_" + osNR.str() + "_binsZ_" + osNZ.str() + "_NRAN_"
    + osNRAN.str() + "_nsamp_" + osSAMPM.str() + "_zmax_" + osZMAX.str() + "_NT_" + osNTASK.str() 
    + "_one_task_" + argv[2] + ".dat";
  const string path_out_GM = path_gen + "CORR_GM/DELTA_SIGMA/ONE_TASK/V2/";
  const string file_out_GM = path_out_GM + "delta_sigma_gm_no_rhob_sub_" + argv[1] + "_one_task_" + argv[2] + ".dat";*/

  const string path_in_GM = path_gen + "CORR_GM/WEIGHTED/";
  const string file_in_GM = path_in_GM + "CFw_snap_" + osSNAP.str() + "_sub_" + argv[1] + "_binsM_" + osNM.str() 
    + "_binsR_" + osNR.str() + "_binsZ_" + osNZ.str() + "_NRAN_" + osNRAN.str() + "_nsamp_" + osSAMPM.str() 
    + "_zmax_" + osZMAX.str() + "_NT_" + osNTASK.str() + ".dat";
  const string path_out_GM = path_gen + "CORR_GM/DELTA_SIGMA/V2/";
  const string file_out_GM = path_out_GM + "delta_sigma_gm_no_rhob_sub_" + argv[1] + ".dat";
  ...
  for(int ib=0; ib<NR; ib++) {
    ...
    for(int im=0; im<NM; im++) {
      double b2, b3, b4, b5, b6;
      in2>>b2>>b3>>b4>>b5>>b6;
      Wgm[ib][im]=b6; //want the wpp with the W integral constraint 
    }
  }
  in2.close();
  ...
  for(int im=0; im<NM; im++) {
    ...
    Ggm[0][im]=exp(OI(log(rbM[1])))-Wgm[0][im];//Ggm[1]; //APPROXIMATION HERE, GIVEN HOW SMALL THE BINS ARE IN THIS REGION         
